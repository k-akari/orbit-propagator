...
contains
  ...
  SUBROUTINE DLEGENDRE(x,lmax,mmax,Plm,dPlm)
    ! compute REAL Associate Legendre Function P_l^m(x), as well as optional
    ! derivatives, for all L up to lmax and all M up to mmax.
    ! Returns results in array Plm, which should be dimensioned as REAL
    ! with size (0:10,0:10).
  ...
  SUBROUTINE SetModel(angle,Bt,Tilt,SWVel,SWDen,ALindex,UseAL)
    ...
    If(First)Then
       ...
       unit=99
       ! MCF   OPEN(UNIT=unit,FILE=cfile,STATUS='OLD',form='UNFORMATTED')
       OPEN(UNIT=unit,FILE='input/w2k_ascii.dat',STATUS='OLD')   
       !       READ(unit) Copyright                      
       !       PRINT *,Copyright                         
       ! MCF end
